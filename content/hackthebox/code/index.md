...
```
Looks like there's an SSH and HTTP service running. The SSH service is usually not vulnerable with HTB, so we can enumerate the HTTP service first.
...
Oh, did I mention "globals" isn't blacklisted :)? I guess the lesson is, if you are going to make a blacklist, you better make sure you blacklist *everything*. Better yet, just do something other than a blacklist -_-.
...
## Privesc to martin
The first thing I saw was that we have access to the app's backend. Usually, I would upgrade my shell, but the connection dies too often for that to matter. The connection also dies too quickly for us to properly do on-net analysis, thus we zip the entire directory and transfer it to our attack host to take our sweet time.
...
While getting the lay of the land, we come across some sudo permissions.
...
With directory traversal, we can have our path start with either `/var/` or `/home/`, then traverse to any location we want. This means we can backup `/root`!
